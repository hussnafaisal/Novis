...
export async function GET(_: Request,{params}:{params:Promise<{id:string}>}){
  ...
  if(!await requireRole("ADMIN"))return new Response("Forbidden",{status:403}); const orders:any[]=await db.order.findMany({}); const order=orders.find(o=>o.invoiceId===id); if(!order)return new Response("Invoice not found",{status:404});
